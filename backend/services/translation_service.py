...
class Translator:
    ...
    def _translate_batch(self, texts):
        ...
        for i, batch in enumerate(batches, 1):
            print(f"Procesando lote {i}/{len(batches)} ({len(batch)} textos)...")
            ...
            if i < len(batches):
                pause = 3.0 if len(batches) > 5 else 2.0
    ...
    def _translate_single_batch(self, texts):
        ...
        system_prompt = f"""Eres un traductor profesional especializado en la industria del cartón ondulado.
        ...
        try:
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                for i, slide_file in enumerate(slide_files, 1):
                    ...
                    try:
                        textos_traducidos = self._update_texts(info["parrafos"], todas_traducciones)
