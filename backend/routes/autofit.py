...
STORAGE_DIR = Path(os.environ.get("AUTOFIT_STORAGE_DIR", "./storage/autofit"))
...
async def process_autofit(file_id: str = Form(...), original_name: str = Form(None)):
    ...
    try:
        ...
        if not Path(result_path).exists():
            raise HTTPException(status_code=500, detail=f"El procesamiento falló: No se generó el archivo")
...
async def process_batch(input_dir: str = Form(None), output_dir: str = Form(None), background_tasks: BackgroundTasks = None):
...
async def get_batch_status(job_id: str):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Error al leer los resultados: {str(e)}") 